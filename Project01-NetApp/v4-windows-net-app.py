...
def get_ping_command(target, mode):
    ...
    if platform.system() == "Windows":
        return ["ping", "-n", "4", target] if mode == "Standard Ping" else ["ping", target]
    else:  # macOS/Linux
        return ["ping", "-c", "4", target] if mode == "Standard Ping" else ["ping", target]


def get_traceroute_command(target):
    """Get the traceroute command based on the operating system."""
    if platform.system() == "Windows":
        return ["tracert", target]  # Windows uses tracert
    else:  # macOS/Linux
        return ["traceroute", target]


def get_whois_command(target):
    """Get the whois command based on the operating system."""
    if platform.system() == "Windows":
        return ["whois", target]  # Ensure whois is installed and in PATH
    else:  # macOS/Linux
        return ["whois", target]


def get_nslookup_command(target):
    """Get the nslookup command."""
    return ["nslookup", target]  # Same for both platforms
...
class NetworkUtility(QMainWindow):
    ...
    def run_ping(self):
        ...
        command = get_ping_command(target, mode)
